...
class RewardBasedEvaluation implements EvaluationMethod
{
	...
	public function evaluateMoves(listOfMoves:ListOfMoves):EvaluationResult
	{
		...
		var currentCreature = GameContext.instance.currentCreature;
	...
	{
		if(currentCreature.totalHealth > 0)
			return ALIVE_REWARD;
		else
			return -ALIVE_REWARD;
	}
	
	private function totalEnemyHealth(totalEnemyPast:Float,enemies: Array<Creature>) : Float
	{
		var proc = calculateEnemyHealth() / totalEnemyPast;
	...
	private function enemiesInAttackRange(enemies: Array<Creature>) : Float
	{
		...
		for (enemy in enemies)
		{
			if (range.exist(enemy.getTileId()))
				countOfEnemies++;
		}
	...
	private function enemiesThatCanAttackMe((enemies: Array<Creature>) : Float
	{
		...
		for (enemy in enemies)
		{
			...
			var distance = HexCoordinates.getManhatanDistance(currentCreature.getTileId(),enemy.getTileId());
	...
	{
		var proc = (Float) currentCreature.totalHealth / myPastHealth;
